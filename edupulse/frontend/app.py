...
if st.button("Voorspel uitval"):
    for idx, row in dff.iterrows():
        ...
        # else:
            # st.markdown(f":blue[**{row['Naam']}**] ({row['Opleiding']} / {row['Klas']}): Lage kans op uitval: {result['probability']:.1%}")
    ...
    if risicostudenten:
        ...
        for row, result in risicostudenten:
            st.markdown(f"**Bezig met genereren van uitleg risico voor** :red[**{row['Naam']}** ({row['Opleiding']} / {row['Klas']}): Kans op uitval: {result['probability']:.1%}]")
            ...
            st.info(f"### Uitleg uitvalrisico **{row['Naam']}** ({row['Opleiding']} / {row['Klas']}): Kans op uitval: {result['probability']:.1%}")
            ...
            try:
                ...
                print("TODO feature_importance")
                st.caption(f"Belangrijkste risicofactoren (SHAP): #TODO")
