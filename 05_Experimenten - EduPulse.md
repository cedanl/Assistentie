# 001 | Experimenten - EduPulse

![Flow experiment 001](https://github.com/cedanl/Assistentie/blob/main/src/assets/20260219_edf_use_case.png)

**Riskante aanname**
_We geloven dat…_
* Uit de (huidige) ‘Uitnodigingsregel’ -en de eventuele (bron)data voor de voorspelling- een interventieplan te generen is voor de desbetreffende (leven-lang-)lerende.



**Weerlegbare hypothese**
_We verwachten dat…_
* We voor de desbetreffende (leven-lang-)lerende een interventieplan kunnen generen uit de (huidige) ‘Uitnodigingsregel’ -en de eventuele (bron)data.


**Experiment**
_Om dit te verifiëren zullen we…_
* Een prototype bouwen dat -in Markdown- een interventieplan genereert voor de (leven-lang-)lerende die driegen uit te vallen.


**Metric**
_En meten we..._
* Dat zo’n interventieplan voor 100 (leven-lang-)lerenden te genereren is die een uitvalpercentage hoger dan 50% hebben. 


**Succes criteria**
_We hebben gelijk als…_
* Er voor 70 van die (leven-lang-)lerenden een interventieplan met minimaal 2 unieke relevante interventies te genereren is.



# 002 | Experimenten - EduPulse


**Riskante aanname**
_We geloven dat…_
* ...



**Weerlegbare hypothese**
_We verwachten dat…_
* ...


**Experiment**
_Om dit te verifiëren zullen we…_
* ...


**Metric**
_En meten we..._
* ...


**Succes criteria**
_We hebben gelijk als…_
* ...
