...
- Depends on [`student-signal`](https://github.com/cedanl/student-signal) (CEDA library, installed via `git+https://`) for KNN-imputation, encoding, and RandomForest training (GridSearchCV); feature lists are stored as JSON next to each model file
